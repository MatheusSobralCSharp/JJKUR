...
import com.jujutsu.jujutsucraftaddon.network.AltarMessageMasteryTwo;
...
public class GreatScreen extends Screen {
    ...
    private List<AltarOption> getOptions() {
        ...
        if (player == null) return options; // Ensure player exists before proceeding
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        ...
        if (mc.player == null) return super.keyPressed(keyCode, scanCode, modifiers);

        KeyMapping input = mc.options.keyUp;  // Example key (W key for moving forward)
    ...
    public boolean keyReleased(int pKeyCode, int pScanCode, int pModifiers) {
        if (pKeyCode == JujutsucraftaddonModKeyMappings.KEY_WHEEL.getKey().getValue()) {
            this.onClose();
        }
    ...
    public void render(@NotNull GuiGraphics guiGraphics, int mouseX, int mouseY, float delta) {
        ...
        for (int i = 0; i < this.getCurrent().size(); i++) {
            ...
            int black = toRGB24(0, 0, 0, 150);  // Pure black
            int white = toRGB24(255, 50, 50, 150);
...
//            Minecraft.getInstance().player.input.tick(false, 0);
//        }
//    }
    ...
    private static class AltarOption {
        ...
        void onSelect() {
            JujutsucraftaddonMod.PACKET_HANDLER.sendToServer(new AltarMessageMasteryTwo(this.index, this.playerUUID));
        }
    }
}
